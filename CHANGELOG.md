...
- **Import references from PDF files** — from the <kbd>n</kbd> "New Entry" menu, choose "Import from PDF" to create entries from one or more PDFs automatically (e.g. clearing out an old downloads folder or a migrated Papers/Zotero library). The picker reuses "Add PDF"'s browse-and-filter list (defaults to the configured download directory; paste a different folder path in to browse elsewhere) but as a checklist, so several files can be picked at once instead of one per entry. Each selected PDF is then scanned for a DOI or arXiv id (embedded document metadata first, then the first two pages of text), the identifier is validated and its metadata fetched through the same CrossRef pipeline as "Import by DOI". Nothing is written until you confirm: a report screen lists every file with a ✓/✗ mark and the reason (an ambiguous file's candidate DOIs are shown too, to copy out and use "Import by DOI" with yourself); pressing <kbd>Space</kbd> on any row, success or failure, previews its PDF; one "Import N Entries" click then commits every ✓ row at once — which PDF to try was already decided by picking it, so there's no second yes/no per file here.
...
- **Open the online documentation** — press <kbd>Ctrl</kbd>+<kbd>d</kbd> (shown in the footer, next to <kbd>?</kbd> Help) to open the docs site in your default browser. Also linked from the `?` help screen's about section.
- **Import a `.bib` file** — from the <kbd>n</kbd> "New Entry" menu, choose "Import .bib File" (<kbd>b</kbd>) and pick a file, e.g. the citation download from a journal page. The picker is the same browse-and-filter list as "Import from PDF"/"Add PDF" (defaults to `~/Downloads`; paste a folder path in to browse elsewhere; <kbd>Space</kbd> previews the highlighted file, <kbd>Enter</kbd>/<kbd>x</kbd> choose it), just for a single `.bib` file instead of a PDF checklist. A file with one entry is added directly — no review screen, same as "Import by DOI". A file with several entries shows a report first (✓ new / ✗ already in your library, by DOI — including a duplicate DOI within the same file) and one "Import N Entries" click commits every ✓ row; entries without a DOI are always treated as new. Reuses the same parser that loads the main library, so there's no size limit beyond what a `.bib` file can hold.

### Changed

- **Consolidated the four ways to add an entry — now five — behind a single <kbd>n</kbd>** — "New" opens a small chooser instead of DOI import and PDF import each having their own top-level keybinding (previously <kbd>d</kbd> and <kbd>i</kbd>, now removed). Pick a way with <kbd>d</kbd> import by DOI, <kbd>p</kbd> import from PDF, <kbd>b</kbd> import a `.bib` file, <kbd>v</kbd> paste BibTeX, or <kbd>m</kbd> fill out manually — listed in that order, DOI import first as the most common way in, manual entry last (the original <kbd>1</kbd>–<kbd>5</kbd> also still work, by letter not position; paste's mnemonic moved from <kbd>b</kbd> to <kbd>v</kbd> to make room for the `.bib` file import). Pasting BibTeX text is still auto-detected anywhere in the app too, in addition to being an explicit chooser option.
...
- **Trimmed and reordered the footer** — it now shows, left to right: New, Edit, Keywords, State, Urgency, Rating (<kbd>0</kbd>–<kbd>5</kbd>, grouped under one "Rating" label), Search, PDF, <kbd>Space</kbd> Show PDF, Quit, Write, Docs, Help, with the command palette hint still docked on the right. Max table, View, Browser, and OpenAlex are no longer shown in the footer — all still work exactly as before and remain listed in the <kbd>?</kbd> help screen.
- **PDF actions consolidated behind a single <kbd>p</kbd>, consistent with the <kbd>n</kbd> New Entry chooser** — Fetch, Add, Copy PDF, Copy path and Delete (previously scattered across two hidden top-level keys and a mouse-only button panel in the detail pane, with Copy/Delete reachable no other way) now live in one "PDF Actions" menu: press <kbd>p</kbd>, then a one-letter mnemonic (<kbd>o</kbd> open, <kbd>f</kbd> fetch, <kbd>a</kbd> add, <kbd>c</kbd> copy PDF, <kbd>p</kbd> copy path, <kbd>d</kbd> delete) chosen instantly, no separate confirm step. All six are always listed, in the same order — same as the old button panel always showing all six buttons — with the ones that don't apply to the entry's current PDF state grayed out rather than removed from the list. <kbd>Space</kbd> is unchanged and still opens the PDF directly. The detail pane's PDF Actions button panel is removed; its status text is now a compact "PDF: ■ linked / □ missing / — none" label alongside read state, urgency and rating.
  - This freed up <kbd>p</kbd>, which used to cycle priority — that's now <kbd>u</kbd>. The concept itself is renamed too, everywhere it's shown: "Priority" is now **"Urgency"** — the footer label, the detail-pane label, the "Cycle priority" help-screen entry, and the column's name in "Table: Configure columns" all read "Urgency" now. The underlying `.bib` field is unchanged (still `priority` / `prio1`–`prio3`, for JabRef compatibility) — only what bibtui calls it in the UI changed.
- **The keywords editor (<kbd>k</kbd>) now toggles on <kbd>Enter</kbd>/<kbd>x</kbd> instead of <kbd>Space</kbd>** — consistent with every checklist in the app (e.g. "Import from PDF"'s file picker): <kbd>Space</kbd> is reserved app-wide for previewing, and there's nothing to preview in a keyword list, so it's now simply inert there rather than toggling. <kbd>Enter</kbd> still adds a newly typed keyword when the filter box has focus, same as before.
- **"Add PDF", "Import from PDF" and "Import .bib File" now open with the list focused, not the filter** — the first row is highlighted, so <kbd>Enter</kbd>/<kbd>x</kbd> chooses it immediately without an extra Tab/Down first, the common case when there's just one obvious file to pick. Press <kbd>s</kbd> from the list to jump to the filter/path box, matching the main view's Search key; typing still works normally once there (or opens straight into it when the folder is empty, same as before). The keywords editor is the deliberate exception — it still opens with the filter focused, since searching or typing a brand-new keyword is the more common first move there.
- **Internal cleanup, no behaviour change** — "Add PDF", "Import from PDF" and "Import .bib File" now share one file-browsing implementation (`_FileBrowseMixin`) instead of three near-identical copies, and the two import report screens share their ✓/✗ row and "Import N Entries" button-label formatting. The three separate DOI-normalizing helpers were reduced to one (`utils/doi.normalize_doi`, with a `lower=False` option for the one caller — an OpenAlex lookup URL — that needs to preserve a DOI's original case); the auto-fix normalizer that writes a cleaned-up DOI back into your `.bib` is intentionally still separate, since it also strips a `doi:` scheme prefix and must never lowercase what it writes to disk.

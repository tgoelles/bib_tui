...
- **Saved filters** — press <kbd>f</kbd> to open **Filters**: named, permanent searches you can jump back to, à la Papers2 smart collections. Keep one `.bib` file for everything, and use a filter to work inside a slice of it, e.g. "Project X" = keyword `sepp`, published 2010 or later. <kbd>0</kbd>–<kbd>9</kbd> jump straight to a filter, <kbd>w</kbd> writes the current search as a new (or updated) one, <kbd>e</kbd> edits and <kbd>d</kbd> deletes the highlighted one; the active filter is marked `●` and already highlighted when you reopen the menu. Deleting a filter always drops back to "All entries", even one that wasn't active, so you're never left looking at a just-deleted filter's results; "All entries" itself can't be deleted and shows a notice instead of doing nothing. A bar above the search box (`Filter: Project X  k:sepp y:2010-  42 / 1203`) and the app's title bar both show what's active. The search box then refines *within* the active filter, exactly like adding another ANDed term — <kbd>Esc</kbd> clears only that refining search, never the filter itself, so you can't drop out of your current project by accident. The last-active filter is restored on the next start. Filters are stored at `~/.config/bibtui/filters.toml`, shared across every `.bib` file you open. See [Saved filters](https://tgoelles.github.io/bib_tui/guide/search/#saved-filters) in the docs.
- **Open-ended and comparison year searches** — `y:2010-` (2010 or later), `y:-2010` (up to 2010), `y:>2010`, `y:>=2010`, `y:<2010`, `y:<=2010`, alongside the existing `y:2010-2020` closed range.
- **Quoted search values** — `k:"sea ice"` keeps a keyword's embedded space as one term instead of splitting on it.
- **Read-state and urgency search prefixes** — `r:`/`state:` (`r:to-read`) and `pr:`/`priority:`/`urgency:` (`pr:high`), matched exactly against the entry's current value.
